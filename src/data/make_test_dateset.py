...
def create_test_data(name: str):
    ...
    CLUSTERS = 20
    clusters = KMeans(n_clusters=CLUSTERS, random_state=0, n_init="auto").fit_predict(
        df.drop("y", axis=1)
    )
    ...
    for i in range(CLUSTERS):
        ...
        clustered_data.append({"df": cluster_df, "ones": all_elem[1] / all_elem.sum()})

    sorted(clustered_data, key=lambda x: x["ones"])

    for i in range(CLUSTERS):
        pd.DataFrame(clustered_data[i]["df"]).to_csv(
            f"../../data/test/{name}-{i}.csv", index=False
        )
